...
function mapToThumbnail( $thumbnail ){
    ...
    return [
        "thumbnailImage" => function() use ( $thumbnail_url ) { return getResizedImage( $thumbnail_url, 200, 200 );},
        "price" => ( $thumbnail["price"] || 1 )." euro",
        "title" => $title,
...
function mapToProduct( $product ){
    ...
    return array_map( function( $productOption ) use ( $product ){
        $productOption["original"] = (float)$productOption["price"];
...
function getThumbnails() {
    $thumbnailsURL = "https://appdsapi-6aa0.kxcdn.com/content.php?lang=de&json=1&search_text=berlin&currencyiso=EU";
    $data = json_decode ( makeRequest( $thumbnailsURL )["get"](), true );
    // shortening the list to 5 thumbnails
    $thumbnails = array_slice( $data["content"], 0, 5 );
